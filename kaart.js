...
/* ========= Excel-import koppeling (plakken boven "// --- Kleine util ---") ========= */

// Eenvoudige centroid voor Polygon/MultiPolygon (EPSG:4326)
function polygonCentroid(geom) {
  const polys = geom?.type === 'Polygon' ? [geom.coordinates]
              : geom?.type === 'MultiPolygon' ? geom.coordinates : [];
  let areaSum = 0, xSum = 0, ySum = 0;
  for (const poly of polys) {
    ...
    if (a) {
      cx = cx / (6*a);
      cy = cy / (6*a);
      areaSum += Math.abs(a); xSum += Math.abs(a)*cx; ySum += Math.abs(a)*cy;
    }
  }
...
// Parcel toevoegen vanuit BRP-feature + Excel-rij
async function addParcelFromBRP({ feature, excelRow, bodem }) {
  const gp = feature?.properties || {};
  const layer = L.geoJSON(feature.geometry, {
    style: { color:'#1e90ff', weight:2, fillOpacity:.2 }
  }).addTo(map);

  const name = gp.weergavenaam || gp.identificatie || ('BRP ' + (excelRow?.sectorId || ''));

  parcels.push({
    id: uuid(),
    layer,
    name,
    ha: Number(excelRow?.ha || 0).toFixed(2),              // Excel is leidend
    gewasCode:  excelRow?.gewasCode ?? (gp.gewascode || ''), // Excel eerst, BRP fallback
    gewasNaam:  excelRow?.gewasNaam  || gp.gewas || '',
    provincie:  gp.provincie || '',
    grondsoort: (bodem?.bodemsoortNaam || bodem?.grondsoort || ''),
    landgebruik: gp.category || 'Onbekend'                  // BRP heeft dit vaak niet → 'Onbekend'
  });
}

// Luisteren op Excel-import event en percelen plotten
window.addEventListener('rvo:imported', async () => {
  try {
    const rows = Array.isArray(window.__RVO_RAW) ? window.__RVO_RAW : [];
    if (!rows.length) return;

    // Unieke IDs en (optioneel) versies verzamelen
    const ids = [...new Set(rows.map(r => r.sectorId).filter(Boolean))];
    const versies = [...new Set(rows.map(r => r.sectorVersie).filter(Boolean))];
    const jaar = new Date().getFullYear();

    // BRP-features ophalen via jouw Netlify Function (server-side PDOK WFS)
    const url = `/.netlify/functions/brpByIds?ids=${encodeURIComponent(ids.join(','))}`
              + (versies.length ? `&versies=${encodeURIComponent(versies.join(','))}` : '')
              + `&jaar=${encodeURIComponent(String(jaar))}`;
    ...
    if (!Object.keys(byId).length) {
      console.warn('[kaart] brpByIds geen matches voor IDs:', ids.slice(0,5), '…');
      alert('Geen BRP-percelen gevonden voor de aangeleverde Sector IDs.');
      return;
    }

    // Voor elke Excel-rij: feature zoeken, bodemsoort via centroid, en toevoegen
    for (const row of rows) {
      const feat = byId[row.sectorId];
      if (!feat) continue;
    ...
    // Lijst opnieuw renderen en berekening trigggeren
    renderParcelList();
    dispatchParcelsChanged();

    // Optioneel: zoom naar alle geselecteerde percelen
    try {
      const group = L.featureGroup(parcels.map(p => p.layer).filter(Boolean));
      if (group.getLayers().length) map.fitBounds(group.getBounds().pad(0.1));
    } catch {}
