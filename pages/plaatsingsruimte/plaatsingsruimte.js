...
// Rewritten: robuuster event-handling, consistente UI-resets, stap-2 navigatie, kleine fixes.
...
// Zorg dat Leaflet de containermaat goed pakt
...
let suppressNextGlobalDeselect = false; // negeert eerstvolgende document-click
let suppressNextMapDeselect    = false; // negeert eerstvolgende map-click
...
function scrollMapIntoView() {
  try { mapEl?.scrollIntoView?.({ behavior: 'smooth', block: 'start' }); } catch {}
}
...
function selectParcel(id, { center = true, scroll = true, scrollMap = false } = {}) {
  ...
  if (scrollMap)  scrollMapIntoView();
}
...
function setAddButtonLoading(isLoading, progressText) {
  const btn = document.querySelector('#kmz-add');
  const labelEl = document.getElementById('kmz-label');
  if (!btn || !labelEl) return;

  btn.removeAttribute('aria-busy');
  btn.classList.remove('is-busy','busy','loading','is-loading');
  const sp = btn.querySelector('.btnspin'); if (sp) sp.remove();

  if (isLoading) {
    btn.disabled = true;
    btn.setAttribute('aria-busy','true');
    labelEl.textContent = progressText || 'Koppelen…';
  } else {
    btn.disabled = false;
    btn.removeAttribute('aria-busy');
    labelEl.textContent = 'Koppel percelen';
  }
}
...
function renderParcelList() {
  ...
  // Sorteer op opp. (ha) aflopend, dan naam
  ...
  sorted.forEach(p => {
    ...
    div.innerHTML = `
      <div class="title-row" style="display:flex; align-items:center; justify-content:space-between; gap:.5rem;">
        <h3 class="parcel-title" style="margin:0; font-size:1rem; line-height:1.2;">${hName}</h3>
        <div class="badge-row" style="display:flex; gap:.3rem; flex-wrap:wrap;">${renderBadges(p.badges)}</div>
      </div>

      <div class="meta-list" style="display:flex; flex-wrap:wrap; gap:.75rem; margin-top:.35rem; color:#555;">
...
// Zichtbaarheid + kaart-dim/highlighten
function applyVisibility(filterKey = 'all', search = '') {
  ...
  if (container) {
    ...
    if (!empty) {
      ...
      empty.innerHTML = `
        <div style="padding:.75rem 0; color:#666;">
          <div class="empty-title" style="font-weight:600;">Geen zoekresultaten</div>
          <div class="empty-hint"  style="font-size:.9rem;">Pas je filter of zoekopdracht aan.</div>
        </div>
      `;
...
/* ---------------------------------
   ...
   (Luistert naar 'rvo:imported' en gebruikt window.__KMZ_RAW + window.__KMZ_GEO)
--------------------------------- */
window.addEventListener('rvo:imported', async () => {
  try {
    ...
    if (!rows.length || !geo) {
      console.warn('[kaart] rvo:imported ontvangen maar geen KMZ-rows/geo aanwezig.');
      return;
    }
    ...
    for (const row of rows) {
      ...
      // bodemsoort lookup (best effort, niet blokkerend)
      let bodem = {};
      ...
      try {
        layer.on('click', (evt) => {
          ...
          suppressNextMapDeselect = true; setTimeout(() => { suppressNextMapDeselect = false; }, 0);
          if (selectedParcelId === id) deselectParcel({ repaint: true });
          else selectParcel(id, { center: true, scroll: true });
        });
      } catch {}
...
/* ---------------------------------
   11) Stap 2 navigatie (nieuw)
--------------------------------- */
function initGoToStep2() {
  const btn = document.getElementById('go-to-step2');
  if (!btn || btn.dataset.bound) return;

  btn.addEventListener('click', () => {
    const count = parcels.length;
    if (!count) {
      alert('Voeg eerst percelen toe voordat je naar stap 2 gaat.');
      return;
    }

    // Optioneel: bewaar laatste stand lokaal (handig voor mestplan als fallback)
    try {
      const minimal = parcels.map(p => ({
        id: p.id, name: p.name ?? null, ha: Number(p.ha)||0,
        gewasCode: p.gewasCode ?? null, gewasNaam: p.gewasNaam ?? null,
        grondsoort: p.grondsoort ?? null, badges: p.badges ?? null,
        centroid: p.centroid ?? null, geometry: p.geometry ?? null
      }));
      localStorage.setItem('plaatsingsruimte:lastParcels', JSON.stringify(minimal));
    } catch {}

    // Als berekening.js al resultaten heeft ingevuld, lees ze en geef mee als query
    const nD = Number(document.getElementById('res_n_dierlijk')?.value || '');
    const nT = Number(document.getElementById('res_n_totaal')?.value || '');
    const pT = Number(document.getElementById('res_p_totaal')?.value || '');

    const params = new URLSearchParams();
    if (Number.isFinite(nD) && nD > 0) params.set('totaalA', String(nD));
    if (Number.isFinite(nT) && nT > 0) params.set('totaalB', String(nT));
    if (Number.isFinite(pT) && pT > 0) params.set('totaalC', String(pT));

    const url = params.toString() ? `mestplan.html?${params}` : 'mestplan.html';
