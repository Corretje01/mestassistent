...
async function loadList(){
  ...
  let q = supabase.from('mest_uploads').select('*, user_id, created_at').order('created_at',{ascending:false});
...
function renderItem(r){
  const link = storageLink(r.file_path); // NB: private; admin kan via getSignedUrl ophalen
  return `
    <details data-id="${r.id}">
      <summary><strong>${escapeHtml(r.naam)}</strong> — ${r.mest_categorie}/${r.mest_type} — ${escapeHtml(r.postcode)} — <em>${r.status}</em></summary>
        ...
        <div style="margin-top:.5rem;">
          <label>Status
            <select class="f-status">
              ${['in_behandeling','gepubliceerd','afgewezen'].map(s=>`<option ${s===r.status?'selected':''} value="${s}">${s.replace('_',' ')}</option>`).join('')}
            </select>
          </label>
        </div>
        <div style="margin-top:.5rem;">
          <label>Moderation note</label>
          <textarea class="f-note">${escapeHtml(r.moderation_note||'')}</textarea>
        </div>
...
// Signed URL opvragen voor private file bekijken
function storageLink(path){ return path; }

function bindItemActions(rows){
  rows.forEach(r => {
    const root = document.querySelector(`details[data-id="${r.id}"]`);
    const btn = root.querySelector('.a-save');
    ...
    btnOpen.addEventListener('click', async () => {
      ...
      if (error) { toast('Kon bestand niet openen', 'error'); return; }
